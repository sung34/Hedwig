 ...
 *        - 글자 수가 0일 때, Reply 버튼이 비활성화되는 이벤트
 ...
 *   # More버튼 렌더링 여부를 확인하는 함수는 @/components/card/index.tsx에서 처리할 예정
 *   - More 버튼 클릭 시 
 *     -슬라이더 메뉴 (글 수정, 글 삭제, 취소) 이벤트
 *       - 글 수정 클릭 시 textEditable: true 토글 이벤트
 *       - 글 수정 상태(textEditable) 상태일때 확인, 취소 버튼 나타나는 이벤트
 *       - 글 삭제 클릭 시 확인 alert 띄우기
 *       - 글 삭제 클릭 시 DELETE('/post/:postId') 요청 
 *     -슬라이더 메뉴 (댓글 수정, 댓글 삭제, 취소) 이벤트
